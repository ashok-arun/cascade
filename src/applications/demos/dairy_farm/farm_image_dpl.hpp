...
inline int preprocess_photo(void* in_buf, size_t in_size, void* out_buf, size_t out_size) {
    ...
    cv::resize(mat, mat, cv::Size(352,240));
...
inline VolatileCascadeStoreWithStringKey::ObjectType get_photo_object(const char* key, const char* photo_file, uint64_t photo_id = 0ul) {
    int fd;
    struct stat st;
    void* file_data;
    ...
    // create Object
    VolatileCascadeStoreWithStringKey::ObjectType ret("/dairy_farm/filter/"+frame_name+"_"+std::to_string(get_time()), reinterpret_cast<const char*>(&frame_data),sizeof(frame_data));
