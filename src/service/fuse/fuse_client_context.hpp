...
public:
  ...
  /** TODO: implement updatecontent function 
   *  optimize this by keep local blob cache instead of fetch everytime
   */
    virtual uint64_t read_file(FileBytes* file_bytes) override {
        dbg_default_trace("[{}]entering {}.", gettid(), __func__);
	    ...
	    if(std::is_base_of<typename CascadeType::ObjectType, ObjectWithStringKey>::value){
	      ...
	      reply.to_bytes(file_bytes->bytes);
	      this->blob = Blob(file_bytes->bytes, file_bytes->size);
	    }
	    file_bytes->size = blob.size;
	    file_bytes->bytes = static_cast<uint8_t*>(malloc(file_bytes->size));
	    memcpy(file_bytes->bytes, blob.bytes, file_bytes->size);
	    dbg_default_trace("[{}]leaving {}.", gettid(), __func__);
	    return 0;
        }
        dbg_default_trace("[{}]leaving {}.", gettid(), __func__);
    ...
    virtual uint64_t get_file_size() override {
        dbg_default_trace("[{}]entering {}. [KEYINODE]", gettid(), __func__);
	ShardINode<CascadeType> *pino_shard = reinterpret_cast<ShardINode<CascadeType>*>(this->parent);
        SubgroupINode<CascadeType> *pino_subgroup = reinterpret_cast<SubgroupINode<CascadeType>*>(pino_shard->parent);
	auto result = capi.template get_size<CascadeType>(
							  key,CURRENT_VERSION,true,pino_subgroup->subgroup_index,pino_shard->shard_index);
        uint64_t fsize = 0;
        for (auto& reply_future:result.get()) {
...
public:
  ...
  ObjectPoolPathINode (fuse_ino_t pino, ServiceClientAPI& _capi):
    capi(_capi),
    update_interval (10), // object pool refreshed in 10 seconds. 
    last_update_sec(0){
...
public:
  ...
  ObjectPoolKeyINode(std::string k, fuse_ino_t pino, ServiceClientAPI& _capi) : 
    ...
    virtual uint64_t get_file_size() override {
        dbg_default_trace("[{}]entering {}.", gettid(), __func__);
        auto result = capi.get_size<std::string>(key,CURRENT_VERSION,true);
        uint64_t fsize = 0;
        for (auto& reply_future:result.get()) {
            fsize = reply_future.second.get();
            break;
