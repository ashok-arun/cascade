...
def error_msg(testcase, output, expected):
    msg =  testcase + "(output--expected):"
    msg += "(" + ','.join(output) 
    msg += " -- " + ','.join(expected) + ")"
    return msg
...
def tst_sg_dirs(mnt_dir):
    ...
    assert compare_dirs(mnt_first_level, expected_first_level)
    ...
    for subgroup_type in expected_first_level:
        ...
        if(subgroup_type == 'PersistentCascadeStoreWithStringKey'):
            ...
            error_m = error_msg("PCSS forthlevel", mnt_forth_level, pcss_expected_keys)
    ...
    print("~~~~~~~~~~~~~ TEST1 SubgroupTypes directories PASS!!  ~~~~~~~~~~~")

    
 # Test 2. check the directory for Cascade object_pools
def tst_objp_dirs(mnt_dir):
    mnt_objp_root = os.listdir(mnt_dir + "/" + "ObjectPools")
    error_m = error_msg("object pool test", mnt_objp_root, objp_expected_second_level)
    ...
    for i in range(1,4):
        ...
        error_m = error_msg(f"object pool test{str(i)}", mnt_objp_dir, objp_expected_third_level[i])
    ...
    print("~~~~~~~~~~~~~  TEST2 ObjectPools directories PASS!!  ~~~~~~~~~~~")
                    

# Test 3. check the open and read content of files
def tst_content(mnt_dir):
    ...
    for i in range(1,2):
        ...
        with open(sg_mnt_key, 'r') as fh:
            output = fh.read()
            error_m = error_msg(f"sg content{sg_mnt_key}", output, put_values[i])
    ...
    for i in range(1,4):
        ...
        with open(objp_mnt_key, 'r') as fh:
            output = fh.read()
            error_m = error_msg(f"objp content{objp_mnt_key}", output, put_values[i])
            assert output == put_values[i], error_m
    print("~~~~~~~~~~~~~ TEST3 content PASS!!  ~~~~~~~~~~~")
